...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('LUSID API Proxy: Received fields request');

    const { baseUrl, accessToken, tableName, cacheBust } = await req.json();

    if (!baseUrl || !accessToken || !tableName) {
    ...
    // Use the specific LUSID endpoint for fields
    const endpointPath = `/honeycomb/api/Catalog/fields?tableLike=${encodeURIComponent(tableName)}`;
    ...
    console.log(`LUSID API Proxy: Attempting to fetch fields from ${fullUrl}`);
    ...
    while (retryCount <= MAX_RETRIES) {
      try {
        ...
        if (response.ok) {
          ...
          console.log('LUSID API Proxy: Successfully retrieved fields data');
          ...
          if (Array.isArray(data)) {
            ...
            if (data.fields && Array.isArray(data.fields)) {
              ...
              // Search for array properties that might contain fields
              ...
              if (arrayProps.length > 0) {
                ...
                result = { fields: data[largestArrayProp] };
              } else {
                // If we can't determine fields, just return the data
                result = data;
              }
            }
          } else {
    ...
    return NextResponse.json(
      {
        error: `Failed to fetch fields from LUSID API after ${MAX_RETRIES + 1} attempts`,
        endpoint: `/honeycomb/api/Catalog/fields?tableLike=${tableName}`,
        lastError,
      },
      { status: 502 }
    );
